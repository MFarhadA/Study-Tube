<?php
    ...
    $stmt->execute();
    $dataVideo = $stmt->get_result()->fetch_assoc();

    $stmt->close();

    $sqlmodul = "SELECT 
                title as modul_title,
                moduleID,
                modul
            FROM
                modul
    ...
    $stmt = $conn->prepare($sqlmodul);
    $stmt->bind_param("i", $videoID);
    $stmt->execute();
    $dataModul = $stmt->get_result()->fetch_assoc();

    if (!$dataModul) {
        $dataModul = array('modul_title' => '', 'moduleID' => '', 'modul' => '');
    }

    $moduleID = $dataModul['moduleID'];
...
<head>
    ...
    <title>Edit Video | Study Tube</title>
...
<body>
    ...
    <div id="mainContent" class="main-content">
        ...
        <div class="fixed top-0 w-full bg-white z-50 shadow-sm">
            <div class="flex items-center space-x-5 p-2">
                ...
                <h1 class="font-poppins font-extrabold text-[#40BA6A] text-xl my-auto">Edit Video</h1>
            </div>
        </div>
        ...
        <div class="container mt-12 p-3">
            ...
            <form method="POST" action="uploadVideo.php" enctype="multipart/form-data">
                ...
                <div class="col-rounded-shadow mx-auto p-2 px-4 h-auto w-auto py-3 mb-3">
                    ...
                    <div class="relative flex items-center w-full mb-4">
                        <input
                            ...
                            value="<?php echo $dataVideo['title']; ?>"
                ...
                <div class="col-rounded-shadow mx-auto p-2 px-4 h-auto w-auto py-3">
                        ...
                        <div class="flex space-x-10 p-2 ml-2 items-center">
                            ...
                            <div class="overflow-hidden w-[250px] h-[150px] bg-[#40BA6A] rounded-lg flex items-center justify-center">
                                <video id="videoPreview" class="w-full h-full object-cover" controls>
                                    <source src="<?php echo $dataVideo['video']; ?>" type="video/mp4">
                                    Video tidak dapat diputar.
                                </video>
                                <svg id="defaultVideoIcon" viewBox="0,0,24,24" xmlns="http://www.w3.org/2000/svg" width="64" height="64" stroke-width="1" transform="rotate(0) matrix(1 0 0 1 0 0)" class="hidden">
                            ...
                            <div class="flex-grow ml-4">
                                <span class="font-roboto font-normal text-black">
                                    Rekomendasi ukuran 1280 x 720 pixels.
                                </span>
                    ...
                    <div class="font-poppins text-lg mx-auto mt-4">Thumbnail</div>
                        <div class="flex space-x-10 p-2 ml-2 items-center">
                            ...
                            <div class="overflow-hidden w-[125px] h-[70px] bg-[#40BA6A] rounded-lg flex items-center justify-center">
                                <img id="profileImage" src="<?php echo $dataVideo['thumbnail']; ?>" alt="Preview Gambar" class="w-full h-full object-cover" />
                                <svg id="defaultImage" viewBox="0,0,16,16" xmlns="http://www.w3.org/2000/svg" width="30" height="30" stroke-width="1" transform="rotate(0) matrix(1 0 0 1 0 0)" class="hidden">
                    ...
                    <span class="flex font-poppins text-lg mx-auto mt-4 space-x-2">
                        ...
                        <div class="flex space-x-10 p-2 ml-2 items-center">
                            ...
                            <div class="flex-grow ml-4">
                                <span class="font-roboto font-normal text-black">
                                    Rekomendasi ukuran 4MB atau kurang dengan format PDF atau Dokumen.
                                </span>
                                <div class="flex flex-auto mt-2 space-x-3 w-auto">
                                    <div class="flex flex-col space-y-2">
                                        <div class="flex flex-row space-x-3">
                                            ...
                                            <?php echo $dataModul['modul_title']; ?>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                ...
                <button type="submit" name="edit"
                    class="py-1 px-6 text-white font-semibold text-2xl rounded-lg bg-[#40BA6A] font-poppins cursor-pointer mt-3 text-center">
                    Ubah
                </button>
                <button type="submit" name="deleteVideo"
                    class="py-1 px-6 text-white font-semibold text-xl rounded-lg bg-[#D9534F] font-poppins cursor-pointer mt-3 ml-2 text-center">
                    Hapus Video
                </button>
                <button type="submit" name="deleteModul"
                    class="py-1 px-6 text-white font-semibold text-xl rounded-lg bg-[#D9534F] font-poppins cursor-pointer mt-3 ml-2 text-center">
                    Hapus Modul
                </button>
    ...
    <script>
        ...
        document.getElementById("resetVideoButton").addEventListener("click", function () {
            // Reset video
            const videoPreview = document.getElementById("videoPreview");
            videoInput.value = "";
            videoPreview.src = "<?php echo $dataVideo['video']; ?>";  // Atur ulang video ke nilai semula
        });
    </script>
    ...
    <script>
        ...
        document.getElementById("resetButton").addEventListener("click", function () {
            // Reset thumbnail
            const profileImage = document.getElementById("profileImage");
            thumbnalkInput.value = "";
            profileImage.src = "<?php echo $dataVideo['thumbnail']; ?>";  // Atur ulang thumbnail ke nilai semula
        });
    </script>
    ...
    <script>
        ...
        resetFileButton.addEventListener('click', function() {
            moduleInput.value = '';
            profileBg.style.backgroundColor = '#40BA6A';
            fileNameDisplay.textContent = "<?= $dataModul['modul_title']; ?>";
        });
    </script>
